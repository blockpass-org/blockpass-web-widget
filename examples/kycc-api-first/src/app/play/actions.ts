...
/**
 * IMPORTANT: This value must be unique for each user. The KYCC will throw an error if the same
 * refID is used for multiple users. For a production environment, please use a unique identifier
 * such as a database ID or another identifier specific to your organization.
 *
 * For more info https://docs.blockpass.org/docs/connect/KYCC-Common-Mgmt-Install-Widget#refid-string
 */
  ...
  if (formData.identity_documents) {
    ...
    if (formData.identity_documents.back) {
      try {
        ...
        await detectImageType(bufferBack);
      } catch (err: any) {
        throw new Error(`Selfie error: ${err.message}`);
      }
    }
